...
export async function POST(req: Request) {
  try {
    ...
    console.log(`[ActionV4] Iniciando: ${action} | Ag: ${appointmentId} | Sol: ${solicitationId}`);
    ...
    if (apError) {
      console.error('[ActionV4] Erro agendamento:', apError);
      ...
      const { error: solUpdateError } = await supabaseAdmin
        ...
        .update({ 
          ...
          data: {
            ...
            resolved_by: 'Action Push v4',
            resolve_comment: isApprove ? 'Aprovado via Push v4' : 'Recusado via Push v4'
          }
        })
        .eq('id', solToUpdateId);

      if (solUpdateError) console.error('[ActionV4] Erro sol update:', solUpdateError);
    } else {
      console.warn('[ActionV4] Nenhuma solicitação encontrada para vincular.');
    }

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('[ActionV4] Erro fatal:', error);
