...
self.addEventListener('push', function(event) {
  if (event.data) {
    ...
    // IDs v9 com busca por palavra-chave
    const actions = [
  ...
  if (action.includes('aprov') || action.includes('recus')) {
    ...
    event.waitUntil(
      self.registration.showNotification('V9: Processando...', {
        ...
        if (response.ok) {
          return self.registration.showNotification('✅ Sucesso V9', {
            body: `Botão: "${action}" | Status: ${result.appliedStatus}`,
            icon: '/icon-192x192.png'
          });
        } else {
          return self.registration.showNotification('❌ Erro V9', {
            body: result.error || 'Falha ao processar.',
            icon: '/icon-192x192.png'
          });
        }
      }).catch(err => {
        return self.registration.showNotification('⚠️ Erro de Conexão V9', {
